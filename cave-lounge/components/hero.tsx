...
// SSR-safe: Three.js only runs client-side
const CocktailHero3D = dynamic(() => import('./cocktail-3d'), {
  ssr: false,
...
export default function Hero() {
  ...
  return (
    <section ref={sectionRef} className="relative h-screen overflow-hidden"
      ...
      {/* Subtle vignette on left so text stays readable */}
      <div className="absolute inset-0 pointer-events-none z-10"
        style={{
          background:
            'linear-gradient(90deg, rgba(10,10,10,0.92) 0%, rgba(10,10,10,0.55) 42%, transparent 62%)',
        }} />
      ...
      {/* ── Left: text content ── */}
      <div className="relative z-20 h-full max-w-7xl mx-auto px-8 lg:px-16
                      flex items-center">
        <div className="w-full lg:w-[46%] flex flex-col">

          <div className="h-line-1 mb-4">
            <span className="font-display text-[9px] tracking-[0.55em]
                             text-[#e84800]/80 uppercase">
          ...
          <p className="h-desc font-sans font-light text-white/30
                        leading-loose max-w-[300px] mb-12"
          ...
          <div className="h-ctas flex flex-wrap gap-4">
            <a href="#reservation"
              className="font-display text-[10px] tracking-[0.25em] uppercase
                         px-9 py-4 bg-[#e84800] text-white hover:bg-[#ff5500]
                         transition-colors duration-300"
              style={{ boxShadow: '0 6px 28px rgba(232,72,0,0.3)',
                       animation: 'subtlePulse 3s ease-in-out infinite' }}>
              Reserve Now
            </a>
            <a href="#menu"
              className="font-display text-[10px] tracking-[0.25em] uppercase
                         px-9 py-4 glass text-white/40 hover:text-white/70
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2
                      flex flex-col items-center gap-3 pointer-events-none z-20">
        <span className="font-display text-[8px] tracking-[0.55em]
                         text-white/15 uppercase">scroll</span>
        <div style={{
          width: 1, height: 44,
